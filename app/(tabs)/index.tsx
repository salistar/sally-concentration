...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const ch = await api.getDailyChallenge('concentration');
      if (ch) {
  ...
  const handleChallenge = async () => {
    ...
    try {
      const room = await api.joinDailyChallenge('concentration');
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity style={styles.actionCard} onPress={() => goVariant('triplets-4x6')} activeOpacity={0.85}>
            <LinearGradient colors={['#7C3AED', '#EC4899']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.actionGradient}>
              <Text style={{ fontSize: 30 }}>🎯</Text>
              <Text style={styles.actionTitle}>{t('home.shortTriplets')}</Text>
              <Text style={styles.actionSub}>{t('home.shortTripletsSub')}</Text>
            </LinearGradient>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionCard} onPress={() => goVariant('daily-challenge')} activeOpacity={0.85}>
            <LinearGradient colors={['#10B981', '#059669']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.actionGradient}>
              <Text style={{ fontSize: 30 }}>📅</Text>
              <Text style={styles.actionTitle}>{t('home.shortDaily')}</Text>
              <Text style={styles.actionSub}>{t('home.shortDailySub')}</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
